...
public class ToClient extends TCP{
	...
	private String				nick;
	private final int			MAX_LENGTH_NICKNAME = 255;
	private final int			MAX_LENGTH_TOPIC = 255;
	...
	public void receive(byte[] bytes, int bytesRead){
		...
		case OpCodes.JOIN:
			...
			this.nick = Server.INSTANCE.isNickOccupied(strUTF);
		...
		case OpCodes.CHNICK:
			...
			String newNick = strUTF;
			String oldNick = this.nick;
			
			if (!newNick.equals("") && newNick.length() <= MAX_LENGTH_NICKNAME) {
		...
		case OpCodes.CHTOPIC:
			...
			String topic;
			try {
				topic = new String( pdu.getSubrange(4, pdu.getByte(1)), "UTF-8");
			} catch (UnsupportedEncodingException e1) {
				topic = new String( pdu.getSubrange(4, pdu.getByte(1)) );
			}
			
			if(topic.length() <= MAX_LENGTH_TOPIC){
